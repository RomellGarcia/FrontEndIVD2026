...
const ReportesEntrenador = () => {
  ...
  // Mejores marcas por disciplina (acotado al club del entrenador)
  ...
  useEffect(() => {
    if (user && idClub) cargarMejoresMarcas();
  }, [user, idClub, filtros]);

  // Carga el club del entrenador y, si tiene uno, sus resultados/categorías
  const cargarDatos = async () => {
    try {
      ...
      const club_id = perfilRes.data.entrenador?.club_id;
      const club_nombre = perfilRes.data.entrenador?.club_nombre;
      if (!club_id) {
        setSinClub(true);
        setCargando(false);
        return;
  ...
  // Carga las mejores marcas por disciplina, acotadas al club del entrenador
  ...
  // Aplica los filtros seleccionados a la lista de resultados (ya
  // acotada al club por el propio backend, vía resultadosAPI.getByClub)
      ...
      <Container maxWidth="xl" sx={{ px: { xs: 2, md: 3 }, pb: { xs: 5, md: 7 } }}>
        {sinClub ? (
          <Box sx={{ ...cardSx, mt: { xs: -5, md: -6 }, textAlign: 'center', py: 6 }}>
            ...
            <Typography variant="body2" sx={{ color: COLORS.ink, opacity: 0.75 }}>
              Únete a un club para poder ver el rendimiento de sus atletas.
            </Typography>
          </Box>
        ) : (
        <>
